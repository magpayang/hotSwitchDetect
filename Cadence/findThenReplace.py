...
def findWord(inputWord, targetFile, outputFile):
	f = open(targetFile, 'r')
	ff = f.read()
	indexWord = inputWord
	tempoString = ''
...
def refineArrayRemoveComments(inputArray):
	...
	for i in range(len(stringInEachIndex)):
		print(StringInEachIndex[i])
#############################################################################################
targetFile = 'sys_and_levels.mod'
outputFile = 'sys_and_levels.mod'
indexWordArray = []	#global
stringInEachIndex = []	#global
indexWordArrayFindWordAfterIndex = [] #global

##arrangement ng indexing 'procedure' and 'body' then writing 'procedure_name = <procedure name>'
findWord('procedure', targetFile, outputFile)	#index location of 'procedure'
getStringInEachIndex(indexWordArray, targetFile) #get the full string in each index
refineArray(stringInEachIndex, 'procedure')	#remoce string 'procedure'
refineArrayExtract(stringInEachIndex)	#remove arguments
inputWord = 'body'
indexArray = indexWordArray
findWordAfterIndex(inputWord, indexArray, targetFile)
inputArray = indexWordArrayFindWordAfterIndex
inputString = stringInEachIndex
inputName = 'procedure'
writeAfterIndex(inputArray, inputString, inputName, targetFile, outputFile)
